...
- 生成 BIFF8 格式文件，支持样式（字体/填充/边框/颜色）、合并单元格、行高
- 超链接、自动筛选、页面设置、页眉页脚、冻结窗格
- 公式写入、列宽设置、工作表保护
...
- `WordEncryptor`：AES-128 加密 docx（ECMA-376 Agile Encryption，OLE2 CFB 容器）
...
- `PdfEncryptor`：加密/权限控制（RC4/AES-128/AES-256）
- `PdfSigner`：数字签名（PKCS#7 分离签名 + 可见签名域，手动 ASN.1 DER 无外部依赖）
- 文本提取：逐页/全文，带坐标信息提取
...
| **PPT pptx** | ✅ | ✅ | ✅ | PDF | 含文本框/图片/表格/图表/母版/动画/批注/Section；PPT→PDF 文本映射型已实现 |
...
- pptx 每张幻灯片渲染为 PNG/JPEG 图像（需外部渲染库 SkiaSharp/Docnet.Core，当前抛出 `NotSupportedException`）

### PDF 页面渲染为图片

- PDF 每页渲染为 PNG/JPEG（需外部渲染库）
